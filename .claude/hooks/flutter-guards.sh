...
case "$f" in
  */lib/features/*.dart)
    ...
    elif [ -f "$f" ] && grep -nE '\b(ElevatedButton|FilledButton|TextButton|OutlinedButton|TextField|Card|ListTile|FilterChip|Chip)\(|\.adaptive\(' "$f" >/dev/null 2>&1; then
      msg="Дизайн-система: в $f используется сырой Material-примитив или *.adaptive-конструктор. В lib/features/** берите компоненты по карте из docs/conventions/design-system.md (AppButton, AppTextField, AppCard, AppListTile, AppChip, ...); adaptive-конструкторы запрещены."
    fi
    ;;
esac
