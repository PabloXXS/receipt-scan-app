// Supabase Edge Function: merchant-suggestions
// Контракт: POST { name: string } → { suggestions: Array<{id, name, similarity}> }
...
type MerchantSuggestionsRequest = { name: string };
...
Deno.serve(async (req) => {
  ...
  try {
    const { name }: MerchantSuggestionsRequest = await req.json();
    ...
    // Вычисляем схожесть и фильтруем
    const suggestions: MerchantSuggestion[] = [];

    for (const merchant of merchants || []) {
      const similarity = calculateMerchantSimilarity(name, merchant.name);
      if (similarity > 0.3) {
    ...
    return json({
      suggestions: suggestions.slice(0, 10), // Возвращаем топ-10
    });
  } catch (error) {
    console.error("Error in merchant-suggestions:", error);
